...
docker run --name kibana -e ELASTICSEARCH_HOSTS=http://192.168.48.129:9200 -p 5601:5601 -d kibana:7.4.2
...
查看elasticsearch版本信息: http://192.168.48.129:9200
显示elasticsearch: 节点信息http://192.168.48.129:9200/_cat/nodes
````
## 6、测试 kibana
````
访问Kibana: http://192.168.48.129:5601/app/kibana
````
# 三、初步检索
## 1、_cat
````
...
# POSTMAN输入
http://192.168.48.129:9200/customer/external/1
...
# POSTMAN输入
http://192.168.48.129:9200/customer/external
...
### 3、POST方式指定id
````
# 在customer索引下的external类型下保存2号数据
...

{
    ...
    "_id": "1",
    "_version": 3,
    "_seq_no": 2, //并发控制字段,每次更新都会+1,用来做乐观锁
...
PUT http://192.168.48.129:9200/customer/external/1?if_seq_no=2&if_primary_term=1
{
    "_index": "customer",
    "_type": "external",
    "_id": "1",
        ...
        "root_cause": [
            {
                ...
                "index_uuid": "VcRGqiibRRmiUHl5hsKCsw",
                "shard": "0",
                "index": "customer"
            }
        ],
...
````
### 2、POST更新文档，带有_update
````
POST http://192.168.48.129:9200/customer/external/1/_update
...
````
### 3、POST更新文档，不带_update
````
POST http://192.168.48.129:9200/customer/external/1

在更新过程中，重复执行更新操作，数据也能够更新成功，不会和原来的数据进行对比。
````
### 4、PUT更新文档，
````
PUT http://192.168.48.129:9200/customer/external/1

PUT操作总会重新保存并增加version版本
````
## 5、删除文档&索引
````
DELETE http://192.168.48.129:9200/customer/external/1
{
    "_index": "customer",
    "_type": "external",
    "_id": "1",
...
# 字符串，全文检索(全文检索，最终会按照评分进行排序，会对检索条件进行分词匹配。)
# 最终的查询出address中包含mill或者road或者mill road的所有记录
GET bank/_search
{
  "query": {
    "match": {
...
````
### 4、示例3-query/match_phrase短语匹配
````
# 不拆分字符串进行检索
# 最终的查询出address中包含mill road的所有记录
GET bank/_search
{
  "query": {
    "match_phrase": {
...
````
### 5、示例4-query/multi_math多字段匹配
````
# address或city中包含mill，并且在查询过程中，会对于查询条件进行分词。
...
# 查询gender为F,地址为mill, 但是age不等于38的数据
GET bank/_search
{
  "query": {
    "bool": {
...
3、should: 应该达到should列举的条件, 如果到达会增加相关文档的评分, 并不会改变查询的结果。
如果query中只有should且只有一种匹配规则,那么should的条件就会被作为默认匹配条件二区改变查询结果。
# 查询gender为M, 地址为mill, 但是age不等于18, lastName应该等于Wallace的数据
GET bank/_search
{
  "query": {
    "bool": {
...
1、搜索address中包含mill的所有人的年龄分布以及平均年龄和平均工资，但不显示这些人的详情
GET bank/_search
{
  # 查询出包含mill的
  "query": {
...
下载: https://github.com/infinilabs/analysis-ik/releases/tag/v7.4.2

解压到: /mydata/elasticsearch/plugins
unzip elasticsearch-analysis-ik-7.4.2.zip -d elasticsearch-analysis-ik-7.4.2
chmod -R 777 elasticsearch-analysis-ik-7.4.2

进入docker容器  
docker exec -it 923c057388de /bin/bash
...
<properties>
        ...
        <!--用户可以在这里配置远程扩展字典 -->
        <entry key="remote_ext_dict">http:192.168.48.129/es/fenci.txt</entry>
        <!--用户可以在这里配置远程扩展停止词字典-->
...
3、编写配置类GulimallElasticSearchConfig

4、测试是否整合成功
GulimallSearchApplicationTests类的contextLoads
````
## 2、测试
````
1、保存数据
GulimallSearchApplicationTests类的indexData

2、检索
GulimallSearchApplicationTests类的searchData
````
