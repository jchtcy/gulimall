...
2、复制brand-add-or-update.vue和brand.vue到product下
...
2、新建module,gulimall-third-party
...
4、nacos新建命名空间third-party

5、命名空间中新建配置文件oss.yml
spring:
  cloud:
    alicloud:
      access-key: 子用户AccessKey ID
      secret-key: 子用户AccessKey Serect
      oss:
        endpoint: oss-cn-hangzhou.aliyuncs.com
        bucket: 桶名字
        
6、项目新建bootstrap.properties

7、项目新建application.yml

8、主启动类 @EnableDiscoveryClient
...
11、访问http://localhost:88/api/thirdparty/oss/policy测试

12、前端修改修改组件中el-upload中的action属性，替换成 http:// + 自己的Bucket域名
...
2、给bean(BrandEntity)添加校验注解，并定义自己的message提示

3、改写BrandController的save
...
3、在product里面新建类GulimallExceptionControllerAdvice，用来集中处理所有异常
````
### 5、分组校验功能
````
...
3、在BrandController的save中使用 @Valided({AddGroup.class}
默认情况下，在分组校验情况下，没有指定指定分组的校验注解，将不会生效，它只会在不分组的情况下生效
````
### 6、自定义校验
````
1、编写一个自定义校验注解 common/valid/ListValue

2、新建配置文件 ValidationMessages.properties保存注解信息
...
5、校验注解添加到 BrandEntity 的 showStatus上，进行测试
````
### 7、查询功能修改
````
修改BrandServiceImpl下的queryPage
````
## 3、表关系讲解
### 1、SPU&SKU
````
...
````
![本地路径](../../img/note/三级分类-属性分组-属性表.PNG)
![本地路径](../../img/note/SPU-SKU-属性表.PNG)
## 4、属性分组
### 1、前端
````
1、执行sys_menu.sql新建完所有菜单

2、复制common、product、member文件夹下所有文件
````
### 2、获取分类属性分组
````
请求方式：GET
请求地址：/product/attrgroup/list/{catelogId}
请求参数：
{
   page: 1,//当前页码
   limit: 10,//每页记录数
...
### 3、属性分组新增功能
````
1、修改 product/category/list/tree接口
三级菜单下面也有children（为空）解决方法：设置后端，当children为空时，不返回children字段
...
2、修改AttrGroupController
````
### 5、删除属性分类
````
删除属性分类表 同时删除属性和属性分类关联表
````
## 5、引入mybatis-plus插件
## 6、品牌管理-关联分类
### 1、新增有用数据
...
````
请求方式：POST
请求地址：product/categorybrandrelation/save
...
### 4、品牌名字修改同时修改表数据
```
修改 product/brand/update 接口
```
### 5、分类名字修改同时修改表数据
```
修改 product/category/update 接口
```
## 7、规格参数
### 1、规格参数新增
````
...
请求方式：GET
请求地址：/product/attr/base/list/{catelogId}
请求参数：
{
   page: 1,//当前页码
   limit: 10,//每页记录数
...
### 5、删除属性
````
删除属性表 同时删除属性和属性分类关联表
````
## 8、销售属性
````
修改 product/attr/base/list/{catelogId} 为 product/attr/{attrType}/list/{catelogId}
同时用一个方法查询销售属性和规格参数
注意：销售属性，没有分组信息，所以复用方法的时候，要判断是销售属性还是规格参数

销售属性的其他接口 都复用规格参数的接口
````
## 9、属性分组-关联属性
### 1、获取属性分组的关联的所有属性
````
...
### 2、删除属性与分组的关联关系
````
新建AttrGroupRelationVo

请求方式：POST
请求地址：/product/attrgroup/attr/relation/delete
...
````
请求方式：POST
请求地址：/product/attrgroup/attr/relation
请求参数:
[{
  "attrGroupId": 0, //分组id
  "attrId": 0, //属性id
...
### 1、调试会员等级相关接口
````
1、/member/memberlevel/list 获取所有会员等级(这个方法已经自动生成了，启动会员服务即可)
2、把gulimall-member添加到服务注册中心，然后启动gulimall-member服务
````
### 2、获取分类关联的品牌
````
新建BrandVo
...
请求方式：GET
请求地址：/product/attrgroup/{catelogId}/withattr
响应数据：
{
	"msg": "success",
	"code": 0,
...
````
复制别人的 SpuSaveVo
在common中新建SpuBoundTo、SkuReductionTo用来做远程调用
在common中新建MemberPrice
最后在R之中添加getCode方法，方便判断远程调用是否成功

请求方式：POST
请求地址：/product/spuinfo/save
请求参数: 
{
	"spuName": "Apple XR",
...
2、出现问题SpuInfoDescEntity，mybatis默认主键为自增的，而SpuInfoDescEntity中的主键为自己输入的，所以修改主键注解
...
发现时间格式不对，对时间进行格式化
在product 的application.yml文件中新增配置：
  jackson:
...
### 1、获取spu规格
````
在前端代码 src/router/index.js 文件中，定义 mainRoutes 时在 children 中定义一行路由（40 行）： 
{ path: '/product-attrupdate', component: _import('modules/product/attrupdate'), name: 'attr-update', meta: { title: '规格维护', isTab: true } } 

请求方式：GET
请求地址：/product/attr/base/listforspu/{spuId}
响应数据：
{
	"msg": "success",
	"code": 0,
...
````
请求方式：POST
请求地址：/product/attr/update/{spuId}
请求参数:
[{
	"attrId": 7,
	"attrName": "入网型号",
...
请求方式：GET
请求地址：/ware/purchase/unreceive/list
响应数据：
{
	"msg": "success",
	"code": 0,
...
## 8、完成采购
````
新建PurchaseItemDoneVo,PurchaseDoneVo
